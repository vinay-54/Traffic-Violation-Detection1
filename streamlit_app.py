...
def main():
            ...
            if st.session_state.get('processing', False):
                ...
                while st.session_state.get('processing', False):
                    ...
                    try:
                        ...
                        # Update progress based on actual detector state
                        if app.detector and hasattr(app.detector, 'frame_count') and frame_count > 0:
                            current_frame = app.detector.frame_count
                            progress = min(100, int((current_frame / frame_count) * 100))
                            progress_bar.progress(progress)
                            
                            # Update status text only
                            elapsed = time.time() - start_time
                            status_text.text(f"Processing... {progress}% ({current_frame}/{frame_count} frames) - {elapsed:.1f}s")
    ...
    if st.session_state.get('results'):
        ...
        if output_path and os.path.exists(output_path):
            ...
            if results.get('used_codec') == 'mp4v':
                st.warning("⚠️ **Browser Playback Warning**: System used 'mp4v' codec which may not play in browsers. If the video below is black, please use the 'Download Video' button to watch it on your computer.")
